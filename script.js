...
const quickLookTooltip = document.getElementById('quickLookTooltip');
...
function updateMemoList() {
    ...
    displayedMemos.forEach((memoObj) => { 
        ...
        // --- Quick Look Tooltip Logic ---
        memoContainer.addEventListener('mousemove', (e) => {
            // Only show if not editing, not dragging, and not clicking action buttons
            if (memoContainer.dataset.isEditing === 'false' && !memoContainer.classList.contains('dragging')) {
                
                // Construct inner content safely
                quickLookTooltip.innerHTML = '';
                const titleEl = document.createElement('div');
                titleEl.className = 'quick-look-title';
                titleEl.textContent = memoObj.name.split(' - ')[0]; // Show just the base name
                
                const contentEl = document.createElement('div');
                contentEl.className = 'quick-look-content';
                contentEl.textContent = memoObj.content;

                quickLookTooltip.appendChild(titleEl);
                quickLookTooltip.appendChild(contentEl);

                // --- Replace the positioning logic inside the mousemove listener with this ---
                
                // Calculate position slightly offset from the cursor
                let x = e.clientX + 15;
                let y = e.clientY + 15;

                const tooltipRect = quickLookTooltip.getBoundingClientRect();

                // If it goes off the right edge, flip it to the left of the cursor
                if (x + tooltipRect.width > window.innerWidth) {
                    x = e.clientX - tooltipRect.width - 15;
                }
                // Safety clamp: Never let it go off the left edge of the screen
                x = Math.max(10, x);
                ...
                // Safety clamp: Never let it go off the top edge of the screen
                y = Math.max(10, y);

                quickLookTooltip.style.left = `${x}px`;
                quickLookTooltip.style.top = `${y}px`;
                
                // ----------------------------------------------------------------------------
                
                quickLookTooltip.classList.remove('hidden');
                // Use requestAnimationFrame to ensure display block applies before opacity transition
                requestAnimationFrame(() => {
                    quickLookTooltip.classList.add('tooltip-visible');
                });
            }
        });

        memoContainer.addEventListener('mouseleave', () => {
            quickLookTooltip.classList.remove('tooltip-visible');
            setTimeout(() => {
                if (!quickLookTooltip.classList.contains('tooltip-visible')) {
                    quickLookTooltip.classList.add('hidden');
                }
